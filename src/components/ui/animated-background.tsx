...
export function AnimatedBackground() {
  ...
  // Using standard Tailwind opacity classes for better JIT compatibility
  const particleOpacities = ['opacity-50', 'opacity-60', 'opacity-70', 'opacity-80', 'opacity-[.85]', 'opacity-90']; // Added more variety based on previous user desires
  ...
  return (
    <div className="fixed inset-0 z-[-1] overflow-hidden">
      ...
      <div 
        ...
        style={{ backgroundSize: '200% 200%' }} // Applied inline style for backgroundSize
      />
      
      {/* Grid Overlay */}
      <div 
        className="absolute inset-0 opacity-30 animate-grid-pulse" 
        style={{ // Applied inline styles for backgroundImage and backgroundSize
        ...
        {Array.from({ length: 20 }, (_, i) => (
          <div
            ...
            style={{
              left: `${5 + (i * 4.5)}%`,
              filter: 'blur(0.5px)'
              // Animation delay is now part of the Tailwind class definition
            }}
          />
        ))}
      </div>
