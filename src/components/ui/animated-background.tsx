
'use client'

import { useEffect, useState } from 'react';

export function AnimatedBackground() {
  const [particleKey, setParticleKey] = useState(0);

  // This effect is to force a remount of particles if their animation classes change,
  // ensuring Tailwind JIT picks up dynamically generated animation names if that strategy is used.
  // However, the current Tailwind config uses a single particleDance keyframe.
  useEffect(() => {
    setParticleKey(prev => prev + 1); 
  }, []);

  return (
    <div className="fixed inset-0 z-[-1] overflow-hidden">
      {/* Animated Gradient Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#002455] via-[#003875] via-[#1e3a8a] via-[#003875] to-[#002455] animate-gradient-shift bg-200%" />
      
      {/* Grid Overlay */}
      <div className="absolute inset-0 opacity-30 animate-grid-pulse bg-grid-pattern bg-grid" />
      
      {/* Floating Geometric Shapes */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute w-[300px] h-[300px] top-[10%] left-[-150px] rounded-full bg-gradient-to-br from-[#FDA001] to-[#FDA001]/30 animate-float-1" />
        <div className="absolute w-[200px] h-[200px] top-[60%] right-[-100px] rounded-full bg-gradient-to-br from-[#FDA001] to-[#FDA001]/30 animate-float-2" />
        <div className="absolute w-[150px] h-[150px] top-[30%] right-[20%] rounded-2xl bg-gradient-to-br from-[#FDA001] to-[#FDA001]/30 rotate-45 animate-float-3" />
        <div className="absolute w-[100px] h-[100px] bottom-[20%] left-[10%] rounded-full bg-gradient-to-br from-[#FDA001] to-[#FDA001]/30 animate-float-4" />
      </div>
      
      {/* Dancing Particles */}
      <div key={particleKey} className="absolute inset-0 overflow-hidden">
        {Array.from({ length: 20 }, (_, i) => (
          <div
            key={i}
            className={`absolute rounded-full bg-[#FDA001] animate-particle-dance-${i + 1}`}
            style={{
              width: `${3 + (i % 6)}px`,
              height: `${3 + (i % 6)}px`,
              opacity: `${(50 + (i % 4) * 10) / 100}`, // Tailwind opacity is 0-1
              left: `${5 + (i * 4.5)}%`,
              animationDelay: `${i * 0.5}s` // Tailwind animation class handles the rest
            }}
          />
        ))}
      </div>
    </div>
  )
}
